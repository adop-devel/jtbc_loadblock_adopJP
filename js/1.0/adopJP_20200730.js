...
adoptag.display = function (k) {
    ...
    if(adoptag.directAd){//아톰에서 직광고 응답받은 경우
        if(adoptag.adinfo.existad){//아톰에서 직광고가 있는 경우
            ...
            adoptag.netAdDisplay(k,adopZonecd,adopWidth,adopHeight,adopPosition);
        }
...
//네트워크 광고가 나가야 할 경우
adoptag.netAdDisplay = function(k,z,w,h,p){
    if(p.toLowerCase() != "popup" && p.toLowerCase() != "full" && p.toLowerCase() != "skin" && p.toLowerCase() != "covi"){
        adoptag.netAdWrite(k,z,w,h,p);
    } else if (p.toLowerCase() == 'covi'){
        adoptag.coviAdWrite(k,z);
    }
}
...
//네트워크 광고 노출(직광고 없을경우)
adoptag.coviAdWrite = function(i,z){
    var div = document.getElementById(i);
    if (div){
        var scr = document.createElement('iframe');
        scr.src = '//compass.adop.cc/ST/'+z;
        scr.async = true;
        div.appendChild(scr);
    }
};
